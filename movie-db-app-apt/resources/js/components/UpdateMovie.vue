<template>
    <div>
        <h1 class="display-4 fw-bold lh-1 mb-3 right">Update Movie</h1>
        <p class="small">Use this form to make some changes to a movie</p>
        <div class="row">
            <div class="col-md-9">
                <form @submit.prevent="updateMovie" id="form">
...
export default {
    ...
    created() {
        axios
            .get(`http://localhost:8000/api/movie/${this.$route.params.id}`)
            .then((res) => {
                this.movie = res.data;
            });
    },
    methods: {
        updateMovie() {
            axios
                .patch(
                    `http://localhost:8000/api/movie/${this.$route.params.id}`,
                    this.movie
                )
                .then((res) => {
